...
These are very much like using transparancies with an overhead projector. 
...
Generally we still need our data to be in some sort of tidyformat.
...
Linear models model the relationship betweeen one or more variables and another variable.
...
We also can get infomation from the model with the ```plot()``` function.
